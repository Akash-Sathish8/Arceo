...
def upgrade() -> None:
    op.create_table(
        ...
        # The LLM-only slice of that same forecast and band. `realized_monthly_usd`
        # below measures LLM tokens and nothing else, because tool and infra dollars
        # are estimated rather than observed — so these are the only forecast columns
        # it can honestly be compared against. Recorded on the day for the same reason
        # the rest of this table is: once prices, defaults and the formula move, the
        # LLM-only figure a past forecast used cannot be reconstructed.
        sa.Column("forecast_tokens_usd", sa.Float),
        sa.Column("forecast_tokens_low_usd", sa.Float),
        sa.Column("forecast_tokens_high_usd", sa.Float),
