...
def replay_traces(
    ...
    for entry in normalized:
        ...
        try:
            ...
            # Never silent-ALLOW: an enforce failure during replay is an UNKNOWN
            # outcome, not a pass. Surface it so coverage isn't overstated.
            logger.warning("replay enforce_check failed for %s %s.%s: %s", agent_id, tool, action, e)
            decision = "ERROR"
            matched = None
    ...
    # ALLOW or ERROR (unknown) both count as not-confirmed-protected.
    report.dangerous_actions_unprotected = sum(
        1 for s in dangerous_steps if s.policy_decision in ("ALLOW", "ERROR")
    )

    if dangerous_steps:
        # Only a confirmed BLOCK / REQUIRE_APPROVAL counts as covered.
        covered = sum(1 for s in dangerous_steps if s.policy_decision in ("BLOCK", "REQUIRE_APPROVAL"))
