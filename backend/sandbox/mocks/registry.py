...
class MockState:
    ...
    def __init__(self, custom_data: dict | None = None, tenant_id: str | None = None, seed: int | None = None):
        self.tenant_id = tenant_id
        # Seeded RNG for deterministic mock randomness (used when seed is provided)
        ...
        # Per-simulation LLM mock cache — prevents cross-simulation/tenant contamination
        self.llm_mock_cache: dict[str, dict] = {}
...
def _llm_mock(tool: str, action: str, params: dict, state: MockState) -> dict | None:
    """Generate a plausible mock response using Haiku. Cached per simulation (on MockState)."""
    cache_key = f"{tool}.{action}"
    if cache_key in state.llm_mock_cache:
        return state.llm_mock_cache[cache_key]
    ...
    try:
        ...
        state.llm_mock_cache[cache_key] = result
        return result
