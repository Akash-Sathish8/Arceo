...
- [x] **MED-013** — `backend/main.py` (679–689, 2869–2884). Add a retention/purge job for `audit_log.detail`; move captured prompt/response content off the tamper-evident chain into a separately-retained store. *(shipped 2026-07-29, PR #132 — new purgeable `llm_captures` table (migration 0014) + retention sweep `jobs/purge_llm_captures.py` + per-subject erasure. **Forward-looking:** rows written BEFORE 0014 keep their content inside the append-only table and can only be scrubbed via the break-glass `scripts/scrub_historical_audit_content.py`, which rewrites the chain.)*
...
- [x] **MED-014** — `backend/main.py` (settings GET/POST) + **three** fire sites, not the two cited: `main.py` spend-anomaly + budget-alert reads and `authority/enforcement.py` (`SELECT *`, so it needed the decrypt call only). Encrypt `slack_webhook_url` via `encryption.split`; mask on read. **Validation:** `test_webhook_encryption.py` (12 tests) covers the column end-to-end; the read path returns a masked value. *(shipped 2026-07-29, PR #136 — migration **0015** adds `slack_webhook_url_enc` and the pair is registered in `encryption.ENCRYPTED_COLUMNS`, which is what gets it backfill + master-key rotation for free per HIGH-004. **Stacked on #132** so the alembic chain stays linear `0013→0014→0015` — two migrations off 0013 would be branching heads. Mask keeps host + last 4 chars so an admin can still tell which webhook is configured; **posting the mask back is understood as "unchanged"** because `Settings.tsx` round-trips the GET response, and writing it through would replace the credential with the mask and silently kill every alert — pinned by `test_resaving_the_mask_preserves_the_stored_secret`. Empty still clears, per the pre-existing contract the page states.)*
