...
- [x] **MED-001** — `backend/auth.py` (146–151) + `backend/main.py` (WS auth 4916). Fail closed when the user row is missing/deleted; enforce `token_version` on the WebSocket handshake; add an admin deprovision endpoint. *(shipped 2026-07-29, PR #130 — fail-closed on missing/disabled row, token_version+disabled_at on the WS handshake, admin revoke/restore + team roster, migration 0013)*
...
- [x] **MED-006** — `backend/main.py` (sync LLM handlers 4773/5953/6075 et al.). Move heavy LLM jobs to a background queue or wrap them in an `anyio.CapacityLimiter`. *(shipped 2026-07-29, PR #131 — async wrappers + a dedicated CapacityLimiter, default 8, with a 503 past the queue window. This is the audit's stated INTERIM fix; the background job queue is tracked below.)*
  - **Validation:** a concurrency test shows sync routes stay responsive while N sweeps run.
- [x] **MED-007** — `backend/shared_state.py` (25, 110). Set `socket_timeout`/`socket_connect_timeout`; call Redis via `anyio.to_thread` or an async client. **DONE (PR #131 completed it).** Both clients carry `socket_timeout`/`socket_connect_timeout` (`REDIS_TIMEOUT`, default 2s), pulled forward with PR #127 because the budget gate blocks on a Redis round-trip. The second half shipped in #131: the sync client's calls on async paths (`_global_rate_limit` middleware, the WS handshake) now go through `anyio.to_thread.run_sync`, and `rate_limit_ok` has an explicit fail posture — closed by default, `fail_open=True` only for the broad per-caller limit so a Redis outage costs rate limiting rather than the whole API.
  - **Validation:** a test with a stalled Redis asserts the request path times out rather than hanging.
- [x] **MED-008** — `backend/sandbox/runner.py` (190, 229). Pass `timeout` + `max_retries` when constructing the OpenAI-compatible client; centralize construction. *(shipped 2026-07-29, PR #131 — `openai_client()` helper in `llm_models.py`, `ARCEO_OPENAI_TIMEOUT` default 60s + `max_retries=2`)*
...
- [ ] **MED-006-b (follow-up)** — replace the interim concurrency limiter with a real **background job queue** (Arq on the existing Redis): the seven heavy endpoints return a job id the client polls, so a long LLM job never occupies a request-serving slot at all. Needs matching frontend work — Sandbox, SweepDetail and SimulationDetail all expect synchronous results today — which is why it wasn't folded into the security fix. Overlaps the "No background job queue" item already in `CLAUDE.md`'s tech-debt list.
  - **Validation:** a sweep survives a worker restart and its result is retrievable by job id.
...
- [x] **MED-002** — `backend/main.py` (**5482–5520**, the WS handler; the cited 4911/4916 had drifted). Replace the JWT-in-URL WS auth with a short-lived single-use ticket. **Validation:** access logs no longer contain a usable bearer token. *(shipped 2026-07-30, PR #138 — new `POST /api/ws-ticket` mints an opaque `secrets.token_urlsafe(32)` ticket into Redis (`ARCEO_WS_TICKET_TTL_SECONDS`, default 30) bound to BOTH the caller and the agent id; the socket redeems it with an atomic **GETDEL**, which is what makes it genuinely single-use — GET-then-DEL would let two racing handshakes share one live ticket. **Hard cutover: `?token=` is refused**, which is only free because nothing in the repo ever opened this socket — no frontend, SDK or website code. Redis unreachable fails **closed**, matching `rate_limit_ok`'s default. #130's revocation guarantee is preserved by re-checking `token_version`/`disabled_at` at REDEEM time, not mint time — pinned by a test that mints, then revokes, then connects. No migration: Redis TTL, so this stayed independent of the #132→#136 stack.)*
- [x] **MED-003** — `backend/auth.py` (68–80). Use `hmac.compare_digest`, then delete the SHA-256 fallback so verify fails closed. **Validation:** `test_auth_hardening.py` asserts `verify_password("x", sha256("x"))` is `False`. **Pre-check:** run `SELECT count(*) FROM users WHERE password_hash NOT LIKE '$2%'`; if > 0, escalate to High and migrate those rows first. *(shipped 2026-07-30, PR #137 — branch DELETED rather than hardened with `compare_digest`, which is strictly stronger: there is nothing left to compare. Pre-check re-confirmed 0 rows across `arceo`/`arceo_test`, so it stayed Medium. A refused non-bcrypt hash now logs a WARNING so a locked-out legacy account is diagnosable instead of an unexplained "invalid password"; `$2a$` still verifies and still upgrades to `$2b$`, which is now the only thing that upgrade path does. ⚠️ `test_auth_polish.py::test_legacy_sha256_hash_upgraded_on_login` was asserting the vulnerable behaviour — inverted to `test_legacy_sha256_hash_is_refused` rather than deleted.)*
- [x] **MED-005** — `backend/main.py` (616–648). Key the `/proxy/llm` rate limit on client IP (or authenticated org); gate agent auto-create behind a key. **Validation:** rotating `X-Agent-ID` no longer resets the bucket. *(shipped 2026-07-29, PR #133 — limiter keyed on org+client IP, auto-create gated behind a key, `ARCEO_PROXY_REQUIRE_KEY` now default-on outside dev)*
- [x] **MED-009** — `backend/main.py` (185–198). Cap bytes actually read from the stream; add an ingress body limit. **Validation:** a chunked over-cap body is rejected. *(shipped 2026-07-29, PR #134 — pure-ASGI `BodySizeLimitMiddleware` caps the bytes actually received, not just the declared length)*
- [x] **MED-012** — `backend/main.py` (2444–2460). Enforce per-file and aggregate byte caps before decoding; validate the branch/ref. **Validation:** a large-file repo does not grow worker RSS past the cap. *(shipped 2026-07-29, PR #134 — streamed per-file + whole-scan byte ceilings, branch/ref validated, skips reported in `scan_notes`)*
...
- [~] **MED-015** — `backend/main.py` (**1473**, not the cited 1366–1368). Uniform signup response and timing parity. **Validation:** a test shows identical status/body/timing for existing vs new account. *(timing half shipped 2026-07-29, PR #135 — the duplicate-email branch skipped bcrypt entirely, so it answered in a fraction of the time a real signup took; it now hashes and discards. **Uniform response deferred to MED-015-b** — it needs out-of-band confirmation for the legitimate new user, and `email_utils.send_email` is a no-op unless `SMTP_HOST` is set, with no verification flow to hang it on. A literal uniform response today means nobody can sign up.)*
- [ ] **MED-015-b** — uniform signup response, blocked on email-verification infrastructure (verification-token table + verify endpoint + `SMTP_HOST` configured + a `Login.tsx` pending state). Until then the 409-vs-200 status remains an account-existence signal — which for a security vendor also hints at who its customers are, since `org_name` is the email domain. Pinned by `test_duplicate_signup_still_returns_409_on_purpose` so the gap stays visible.
- [x] **MED-016** — `backend/main.py` (**886/2523/3569/4579** — the cited 791/3102/2282/4106 had all drifted) + `runner.py` (**430**; the cited 396 is system-prompt selection, not an error path). Return a static message + server-side correlation id; log detail server-side only. **Validation:** error responses no longer contain upstream URLs or `str(e)`. *(shipped 2026-07-29, PR #135 — new `backend/errors.py` `log_and_ref()` logs the exception in full against a random 12-hex ref and returns only the ref. The proxy site was logging **nothing**, so redacting without relocating would have destroyed the only copy. The MCP site also stopped echoing the caller's URL. **Two sites deliberately left readable:** `main.py:4119` `VaultConfigError` (operator guidance, no secret material — a bare 503 is unactionable) and `main.py:6300` invoice-CSV parse errors (describes the caller's own uploaded file; opaque means re-uploading blind). Both pinned by tests so they read as decisions, not misses.)*
- [x] **MED-017** — `backend/main.py` (616/5456), `registry.py` (525), `risk_classifier.py` (756). Strip CR/LF/control chars before logging; constrain agent-id to `[a-z0-9-]`. **Validation:** a forged newline in `X-Agent-ID` cannot inject a log line. *(shipped 2026-07-29, PR #133 — `redaction.log_safe()` at all three sinks + `X-Agent-ID` charset validated at ingest)*
- [x] **MED-018** — `.github/actions/scan/action.yml` (45–47). Hash-pin the `httpx` install or use the stdlib. **Validation:** `pip install --require-hashes` succeeds in the action. *(shipped 2026-07-30, PR #137 — took the **stdlib** option: the install step is GONE, so there is nothing to pin or refresh. `run.py`'s two JSON POSTs go through a `_post_json` helper on `urllib.request` that normalises urllib's raise-on-4xx/5xx back to httpx's returned-status shape, which is what every call site branches on. run.py had **zero** test coverage before this — `test_scan_action_transport.py` (7 tests) now pins the transport contract and asserts the action installs nothing. Also folded in: `ci.yml`'s unpinned `pip install pip-audit` → `==2.10.1`, same class, Arceo's own CI rather than a customer's.)*
