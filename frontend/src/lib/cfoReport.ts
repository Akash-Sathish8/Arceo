...
function buildConfidenceLine(tier: "low" | "medium" | "high"): string {
  switch (tier) {
    ...
    case "medium":
      return "Moderate. Based on simulated test conversations. It reaches high confidence if this agent captures " +
        `${HIGH_GATE_CALLS} or more production calls spanning ${HIGH_GATE_DAYS} or more distinct days within any ` +
    ...
    case "high":
      return `High. Based on ${HIGH_GATE_CALLS} or more captured production calls spanning ` +
        `${HIGH_GATE_DAYS} or more distinct days in the trailing ${HIGH_GATE_WINDOW_DAYS}-day window.`
  }
}
