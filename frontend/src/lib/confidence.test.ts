...
describe("no customer-facing surface says the band tightens with time", () => {
  ...
  test("the surfaces that explain the gate say it is a rate", () => {
    // The surfaces that carry the explanation must state the cap, not merely
    // avoid denying it. CFOReport.tsx renders cfoReport.ts's line and is checked
    // there. SpendDashboard.tsx lost its methodology popover in the 2026-09
    // redesign, so it no longer explains the gate; it stays in SURFACES above
    // so it can never re-acquire a time-based promise.
    for (const rel of ["../pages/CostPortfolio.tsx", "../components/FleetCFOReport.tsx"]) {
